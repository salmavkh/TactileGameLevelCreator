...
public class CustomizeBuilder : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (platform == null)
        {
            Debug.LogError($"Missing '{platformObjectName}' in scene.");
            return;
        }
        ...
        if (logPaths)
        {
            Debug.Log("CustomizeBuilder runDir: " + runDir);
        ...
        for (int idx = 0; idx < outersPx.Count; idx++)
        {
            ...
            if (topPx == null || topPx.Count < 2)
            {
                ...
                if (logSkipReasons)
                    Debug.Log($"SKIP Polygon {idx}: BuildTopSilhouettePx returned {(topPx == null ? "null" : $"only {topPx.Count} pts")}.");
                continue;
            }
            ...
            if (topPx == null || topPx.Count < 2)
            {
                skippedAfterCleanup++;
                if (logSkipReasons)
                    Debug.Log($"SKIP Polygon {idx}: after cleanup/simplify, top edge <2 points. (before={beforeCleanup})");
                continue;
            }
        ...
        Debug.Log($"Spawned one-way top-edge pieces (Customize): {spawned}");
        if (spawned == 0)
    ...
    static List<Vector2> BuildTopSilhouettePx(List<Vector2> poly, int samples)
    {
        ...
        return outPts.Count >= 2 ? outPts : null;
    }
